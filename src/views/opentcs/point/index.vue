  ...
  <div class="p-2">
      ...
      <div v-show="showSearch" class="search">
        <el-form ref="queryFormRef" :model="queryParams" :inline="true" label-width="100px">
          <el-form-item label="点位名称" prop="name">
            <el-input v-model="queryParams.name" placeholder="请输入点位名称" clearable @keyup.enter="handleQuery" />
          </el-form-item>
          <el-form-item label="点位编码" prop="code">
            <el-input v-model="queryParams.code" placeholder="请输入点位编码" clearable @keyup.enter="handleQuery" />
          </el-form-item>
          <el-form-item label="点位类型" prop="type">
            <el-input v-model="queryParams.type" placeholder="请输入点位类型" clearable @keyup.enter="handleQuery" />
          </el-form-item>
    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button v-hasPermi="['opentcs:point:add']" type="primary" plain icon="Plus" @click="handleAdd">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermi="['opentcs:point:edit']" type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()">
              修改
            </el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermi="['opentcs:point:remove']" type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()">
              删除
            </el-button>
          </el-col>
      ...
      <el-table v-loading="loading" :data="pointList" border @selection-change="handleSelectionChange">
        ...
        <el-table-column label="点位编码" align="center" prop="code" />
        ...
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="150">
          <template #default="scope">
            <el-tooltip content="修改" placement="top">
              <el-button v-hasPermi="['opentcs:point:edit']" link type="primary" icon="Edit" @click="handleUpdate(scope.row)"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button v-hasPermi="['opentcs:point:remove']" link type="primary" icon="Delete" @click="handleDelete(scope.row)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" width="500px" append-to-body>
      <el-form ref="pointFormRef" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="点位类型" prop="type">
          <el-input v-model="form.type" placeholder="请输入点位类型" />
        </el-form-item>
...
import { listPoint, getPoint, delPoint, addPoint, updatePoint } from '@/api/opentcs/point';
import { PointVO, PointQuery, PointForm } from '@/api/opentcs/point/types';
...
const handleDelete = async (row?: PointVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除点位编号为"' + _ids + '"的数据项？').finally(() => (loading.value = false));
  await delPoint(_ids);
...
/** 状态修改  */
const handleStatusChange = async (row: PointVO) => {
  const text = row.status === '0' ? '启用' : '停用';
  try {
    await proxy?.$modal.confirm('确认要"' + text + '"吗?');
    await updatePoint({ id: row.id, status: row.status });
