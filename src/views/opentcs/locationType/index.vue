  ...
  <div class="p-2">
    <transition :enter-active-class="proxy?.animate.searchAnimate.enter" :leave-active-class="proxy?.animate.searchAnimate.leave">
      <div v-show="showSearch" class="search">
        <el-form ref="queryFormRef" :model="queryParams" :inline="true" label-width="100px">
          ...
          <el-form-item label="位置类型编码" prop="code">
            <el-input v-model="queryParams.code" placeholder="请输入位置类型编码" clearable @keyup.enter="handleQuery" />
          </el-form-item>
    ...
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" :data="locationTypeList" border @selection-change="handleSelectionChange">
        ...
        <el-table-column label="位置类型名称" align="center" prop="name" />
        <el-table-column label="位置类型编码" align="center" prop="code" />
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" width="500px" append-to-body>
      <el-form ref="locationTypeFormRef" :model="form" :rules="rules" label-width="120px">
        <el-form-item label="位置类型名称" prop="name">
          <el-input v-model="form.name" placeholder="请输入位置类型名称" />
        </el-form-item>
        <el-form-item label="位置类型编码" prop="code">
          <el-input v-model="form.code" placeholder="请输入位置类型编码" />
        </el-form-item>
...
const initFormData: LocationTypeForm = {
  id: undefined,
  name: undefined,
  code: undefined,
...
const data = reactive<PageData<LocationTypeForm, LocationTypeQuery>>({
  form: { ...initFormData },
  queryParams: {
    pageNum: 1,
    pageSize: 10,
...
const submitForm = () => {
  locationTypeFormRef.value?.validate(async (valid: boolean) => {
    if (valid) {
      ...
      if (form.value.id) {
        await updateLocationType(form.value).finally(() => (buttonLoading.value = false));
      } else {
        await addLocationType(form.value).finally(() => (buttonLoading.value = false));
      }
...
/** 状态修改  */
const handleStatusChange = async (row: LocationTypeVO) => {
  const text = row.status === '0' ? '启用' : '停用';
  try {
    await proxy?.$modal.confirm('确认要"' + text + '"吗?');
    await updateLocationType({ id: row.id, status: row.status });
